...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        allNoistyDistance = util.Counter()
        ...
        #alpha = 1 / len(emissionModel)

        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            #if emissionModel[trueDistance] > 0: allPossible[p] = 1.0

            proba = emissionModel[trueDistance] * self.beliefs[p]
            allPossible[p] = proba
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for position in self.legalPositions:
            transitionModel = self.getPositionDistribution(self.setGhostPosition(gameState, position))
            #print transitinModel
            for p in transitionModel:
                newBelief[p] += transitionModel[p] * self.beliefs[position]
        newBelief.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        particleNumber = self.numParticles
        legal = self.legalPositions
        self.particles = []
        while particleNumber > 0:
            if particleNumber > len(legal):
                self.particles += legal
                particleNumber -= len(legal)
            else:
                self.particle = legal[0: (particleNumber -1)]
                particleNumber = 0
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                newDist[p] += emissionModel[trueDistance] 
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # permutatinLength = len(permutations)
        # n = self.numParticles
        # self.particles = []
        # while n > permutatinLength:
        #     self.particles += permutations
        #     n -= permutatinLength
        # self.particles += permutations[0: n - 1]
        prod = itertools.product(self.legalPositions, repeat=self.numGhosts)
        prod = list(prod)
        random.shuffle(prod)
        particles = []
        i = j = 0
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for g in range(self.numGhosts):
            if noisyDistances[g] == None:
                for p in range(self.numParticles):
                    self.particles[p] = self.getParticleWithGhostInJail(self.particles[p], g)
        ...
        for p in self.particles:
            ...
            for g in range(self.numGhosts):
                if noisyDistances[g] != None:
                    dist = util.manhattanDistance(p[g], pacmanPosition)
                    weight *= emissionModels[g][dist]
            weighted[p] += weight
        if weighted.totalCount() != 0:
            self.particles = [util.sample(weighted) for k in range(self.numParticles)]
        else:
            self.initializeParticles()
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                prev = list(oldParticle)
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, prev), i, self.ghostAgents[i])
